...
test_that("independent data is not rejected", {
    ...
    result = modified_likelihood_test(x, y, z, FALSE, 0.05)
...
test_that("dependent data is rejected", {
    ...
    result = modified_likelihood_test(x, y, z, FALSE, 0.05)
    expect_equal(result$kind, "statistic")
    expect_true(abs(result$statistic - 7.053439978825427) < 1e-9)
    expect_true(abs(result$p_value - 0.007911317670556329) < 1e-12)
    expect_equal(result$df, 1)
  })
...
test_that("boolean mode returns independent=FALSE for dependent data", {
    ...
    result = modified_likelihood_test(x, y, z, TRUE, 0.05)
