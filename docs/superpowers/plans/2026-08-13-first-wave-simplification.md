...
- Keep the R package's synchronized `ci-core`, package fixture, lockfile, and vendor archive self-contained and offline-buildable.
...
- Modify: `crates/ci-js/Cargo.toml`
- Modify: `crates/ci-r/src/rust/Cargo.toml`
- Modify: `crates/ci-r/src/rust/Cargo.lock` (generated)
- Modify: `crates/ci-r/src/rust/ci-core/Cargo.toml` (synchronized)
- Modify: `crates/ci-r/src/rust/vendor.tar.xz` (generated)
...
Expected: FAIL because `default-features` is absent and `ci-js` declares both `getrandom` versions.

- [ ] **Step 3: Make the minimal manifest changes**

Use this workspace dependency in both `Cargo.toml` and `crates/ci-r/src/rust/Cargo.toml`:

```toml
statrs = { version = "0.18.0", default-features = false }
```

Respect each file's existing quote style. Remove the unconditional and target-specific `getrandom` entries from `crates/ci-js/Cargo.toml`; remove the now-empty target dependency table.
...
cargo generate-lockfile --manifest-path crates/ci-r/src/rust/Cargo.toml
...
cargo vendor --locked --manifest-path crates/ci-r/src/rust/Cargo.toml "$vendor_tmp/vendor"
tar --sort=name --mtime=@0 --owner=0 --group=0 --numeric-owner -C "$vendor_tmp" -cJf crates/ci-r/src/rust/vendor.tar.xz vendor
```

The temporary directory is outside the repository. The archive must contain exactly the registry packages present in `crates/ci-r/src/rust/Cargo.lock`.
...
cargo tree -p ci_core --edges normal
cargo test -p ci_core
cargo clippy -p ci_core --all-targets -- -D warnings
WASM_PACK_CACHE=/tmp/task7-wasm-pack-cache wasm-pack build crates/ci-js --target nodejs
npm --prefix crates/ci-js test
```
...
```bash
git add Cargo.toml Cargo.lock tests/test_release_configuration.py crates/ci-js/Cargo.toml crates/ci-r/src/rust/Cargo.toml crates/ci-r/src/rust/Cargo.lock crates/ci-r/src/rust/ci-core crates/ci-r/src/rust/vendor.tar.xz
...
**Files:**
- Modify: `tests/test_release_configuration.py`
- Delete: `crates/ci-js/tests/package.json`
- Delete: `crates/ci-js/tests/package-lock.json`
- Modify: `.github/workflows/js.yml`

**Interfaces:**
- Consumes: `crates/ci-js/package.json`, its lockfile, and the existing `tests/*.test.js` discovery convention.
...
def test_js_uses_one_npm_project() -> None:
    ...
    for job_name in ("js-lint", "js-test"):
        ...
        assert setup["with"]["cache-dependency-path"] == "crates/ci-js/package-lock.json"
...
Expected: FAIL because both nested npm files exist and the test job points at `crates/ci-js/tests`.

- [ ] **Step 3: Delete the nested npm project and update CI**

Delete the two nested package files. In `js-test`, change `cache-dependency-path` and both npm step working directories to `crates/ci-js`; rename “Install test dependencies” to “Install dependencies”. Update its verification comment to say the root script discovers `tests/*.test.js`. Preserve job IDs, display names, dependencies, and OS matrix.
...
```bash
npm_config_cache=/tmp/ci-tests-first-wave-npm npm --prefix crates/ci-js ci
npm --prefix crates/ci-js test
(cd crates/ci-js && npx prettier --check .)
(cd crates/ci-js && npx eslint .)
...
```bash
git add .github/workflows/js.yml tests/test_release_configuration.py crates/ci-js/tests/package.json crates/ci-js/tests/package-lock.json
...
**Files:**
- Modify: `crates/ci-core/src/ci_tests/discrete_common.rs`
- Modify: `crates/ci-core/src/ci_tests/chi_squared.rs`
- Modify: `crates/ci-core/src/ci_tests/log_likelihood.rs`
- Modify: `crates/ci-core/src/ci_tests/cressie_read.rs`
- Modify: `crates/ci-core/src/ci_tests/freeman_tukey.rs`
- Modify: `crates/ci-core/src/ci_tests/modified_likelihood.rs`
- Synchronize: `crates/ci-r/src/rust/ci-core/**`
...
Run: `cargo test -p ci_core ci_tests::discrete_common::tests::family_members_share_contract`
...
```bash
git add crates/ci-core/src/ci_tests crates/ci-r/src/rust/ci-core
...
- Modify: `crates/ci-core/README.md`
- Modify: `crates/ci-js/README.md`
- Modify: `crates/ci-js/examples/test.html`
- Modify: `crates/ci-core/Cargo.toml`
- Modify: `crates/ci-python/Cargo.toml`
- Modify: `crates/ci-js/Cargo.toml`
- Modify: `crates/ci-js/package.json`
- Synchronize: `crates/ci-r/src/rust/ci-core/Cargo.toml`
...
def test_leaf_docs_examples_and_package_metadata_are_current() -> None:
    ...
    for relative in ("crates/ci-core/Cargo.toml", "crates/ci-python/Cargo.toml", "crates/ci-js/Cargo.toml"):
...
`crates/README.md` should list the four package roles and explain that the root Cargo workspace contains core/Python/JS while R is a standalone source-package workspace. `ci-core/README.md` should describe `Dataset`, `CITest`, the eight tests, and link to root `README.md`/`CONTRIBUTING.md`. `ci-js/README.md` should show the current `Dataset` + class API, `wasm-pack build crates/ci-js --target nodejs`, and root `npm ci && npm test` commands without duplicating the full root guide.

- [ ] **Step 4: Rewrite the browser example against the current API**

Import `Dataset` and `PearsonCorrelation` from `../pkg/ci_js.js`, await `init()`, build two continuous columns, construct `new PearsonCorrelation(data)`, call `runTest("X", "Y", [])`, and render `JSON.stringify(result, null, 2)`. Remove every removed function-style export.
...
Remove `exclude = [""]` from `ci-core`. Replace the npm description with `WebAssembly bindings for data-bound conditional-independence testing` and add the pgmpy repository URL using npm's standard repository object.
...
```bash
git add tests/test_release_configuration.py crates/README.md crates/ci-core/README.md crates/ci-js/README.md crates/ci-js/examples/test.html crates/ci-core/Cargo.toml crates/ci-python/Cargo.toml crates/ci-js/Cargo.toml crates/ci-js/package.json crates/ci-r/src/rust/ci-core/Cargo.toml
...
python crates/ci-r/tools/sync_package_assets.py --check
cmp tests/fixtures/golden.json crates/ci-r/tests/testthat/fixtures/golden.json
```
...
```bash
python -m ruff format --check crates/ci-python
python -m ruff check crates/ci-python
(cd crates/ci-python && mypy test/)
pytest crates/ci-python/test/ -q
```
...
```bash
PATH=/tmp/ci-tests-r-toolchain/bin:$PATH LD_LIBRARY_PATH=/home/ankur/miniconda3/envs/expert/lib/R/lib R_LIBS_USER=/tmp/ci-tests-r-lib CARGO_NET_OFFLINE=true /home/ankur/miniconda3/envs/expert/bin/Rscript -e 'rextendr::document("crates/ci-r")'
git diff --exit-code -- crates/ci-r/R/extendr-wrappers.R crates/ci-r/man crates/ci-r/NAMESPACE
PATH=/tmp/ci-tests-r-toolchain/bin:$PATH LD_LIBRARY_PATH=/home/ankur/miniconda3/envs/expert/lib/R/lib R_LIBS_USER=/tmp/ci-tests-r-lib CARGO_NET_OFFLINE=true /home/ankur/miniconda3/envs/expert/bin/Rscript -e 'lints <- lintr::lint_package("crates/ci-r"); print(lints); quit(status = as.integer(length(lints) > 0L))'
PATH=/tmp/ci-tests-r-toolchain/bin:$PATH LD_LIBRARY_PATH=/home/ankur/miniconda3/envs/expert/lib/R/lib R_LIBS_USER=/tmp/ci-tests-r-lib CARGO_NET_OFFLINE=true /home/ankur/miniconda3/envs/expert/bin/Rscript -e 'devtools::test("crates/ci-r", reporter = "summary")'
PATH=/tmp/ci-tests-r-toolchain/bin:$PATH LD_LIBRARY_PATH=/home/ankur/miniconda3/envs/expert/lib/R/lib R_LIBS_USER=/tmp/ci-tests-r-lib CARGO_NET_OFFLINE=true /home/ankur/miniconda3/envs/expert/bin/Rscript -e 'archive <- devtools::build("crates/ci-r", path = "/tmp/ci-tests-r-first-wave", manual = FALSE, vignettes = FALSE); devtools::check_built(archive, cran = FALSE, manual = FALSE, args = "--no-manual", check_dir = "/tmp/ci-tests-r-first-wave/check", error_on = "warning")'
```
